...
const EventModal = () => {
  ...
  return (
    <div className={`${eventModalIsOpen ? 'fixed' : 'hidden'} top-0 left-0 flex justify-center w-full h-full`}>
      <div className="fixed cursor-pointer w-full h-full" onClick={handleToggleModal} />
        ...
        <div className="p-3">
          <div className="flex flex-col flex-wrap justify-center">
            ...
            <div className="mt-3 flex items-center">
              ...
              <input className="w-24 text-sm" type="date" value={date} onChange={(e) => setDate(e.target.value)} />

              <div className="flex items-center">
                <SingleSelect className="text-sm" options={startSelectOptions} onChange={handleStartSelectClick} selected={startTime} />

                <span className="mx-2">-</span>

                <SingleSelect className="text-sm" options={endSelectOptions} onChange={handleEndSelectClick} selected={endTime} />
              </div>
            </div>

            <div className="flex justify-end p-3 mt-5">
              <Button className="bg-title hover:bg-hover px-6 py-2 rounded text-background" onClick={handleSubmit}>
                저장
              </Button>
            </div>
          </div>
