...
export const useSelect = <TItem = string>({
  ...
  const isSelectedItem = useCallback(
    (item: TItem, level: number) => {
      const selected = selectedItems?.[level] && isEqual(item, selectedItems[level]);
      return !!selected;
    },
  ...
  const handleClickItem = useCallback(
    (item: TItem, level: number) => () => {
      ...
      setRenderingItems((prev) =>
        Object.entries(prev || {}).reduce((updatedrenderingItems, [currentLevel, currentLevelItems]) => {
...
export const useClassNames = (classes?: Partial<Record<ClassName, string>>) => ({
  root: () => clsx('selection-root', classes?.root),
  overlay: (open: boolean) => clsx('overlay', classes?.overlay, { hidden: !open }),
  selectionHeader: () => clsx('select-wrapper', 'selection-header', classes?.selectionHeader),
  ...
  levelItem: ({ nestable, selected }: { nestable: boolean; selected: boolean }) =>
...
const classNames = ['root', 'overlay', 'selectionHeader', 'selectionEntries', 'levelEntry', 'levelItem', 'levelSelectedItem'] as const;
